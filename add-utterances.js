...
var uploadFile = "simple/simple_{n}.json";
var filen = 5;
...
if (process.argv.length >= 3) {
    if (process.argv[2] === "-train") {
        ...
        uploadFile = process.argv[3];
        filen = process.argv[4];
    }
}
...
var configAddUtteranceF = n => {
    return {
        LUIS_subscriptionKey: LUIS_programmaticKey,
        LUIS_appId: LUIS_appId,
        LUIS_versionId: LUIS_versionId,
        inFile: path.join(__dirname, uploadFile.replace("{n}", n)),
        uri: "https://westeurope.api.cognitive.microsoft.com/luis/api/v2.0/apps/{appId}/versions/{versionId}/examples"
            .replace("{appId}", LUIS_appId)
            .replace("{versionId}", LUIS_versionId)
    };
...
var addUtterance = async config => {
    try {
        ...
        let response = await fse.writeJson(
            config.inFile.replace(".json", ".results.json"),
            results
        );
...
var configTrain = {
    ...
    LUIS_appId: LUIS_appId,
    LUIS_versionId: LUIS_versionId,
    uri: "https://westeurope.api.cognitive.microsoft.com/luis/api/v2.0/apps/{appId}/versions/{versionId}/train"
        .replace("{appId}", LUIS_appId)
        .replace("{versionId}", LUIS_versionId),
...
var train = async config => {
    try {
        ...
        if (config.method === "POST") {
            let response = await fse.writeJson(
                path.join(__dirname, "training-results.json"),
                results
            );
            console.log(
                `Training request sent. The status of the training request is: ${
                    results.response.status
                }.`
            );
        } else if (config.method === "GET") {
            let response = await fse.writeJson(
                path.join(__dirname, "training-status-results.json"),
                results
            );
...
var main = async () => {
    if (trainAfterAdd) {
        ...
        addUtterance(configAddUtterance)
            .then(() => {
                console.log(
                    "Add utterance complete. About to request training."
                );
            ...
            .then(() => {
                console.log(
                    "Training process complete. Requesting training status."
                );
        ...
        for (let i = 0; i <= filen; i++) {
            await addUtterance(configAddUtteranceF(i)).then(() => {
